...
const DoctorAppointments = () => {
  ...
  const { slotDateFormat, calculateAge, formatMoney } = useContext(AppContext)
        ...
        {appointments.map((item, index) => (
          <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={index}>
            ...
            <div className='flex items-center gap-2'>
              <AvatarImage src={item.user_data.image} name={item.user_data.name} size="xs" /> <p>{item.user_data.name}</p>
            </div>
            <div>
            ...
            <p className='max-sm:hidden'>{calculateAge(item.user_data.dob)}</p>
            <p>{slotDateFormat(item.slot_date)}, {item.slot_time}</p>
